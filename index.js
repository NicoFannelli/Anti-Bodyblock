module.exports = function antiBodyBlockClassic(mod) {
	...
	mod.hook('S_LOGIN', 'raw', _ => {
		if (enabled) interval = mod.setInterval(removeBodyBlock, 5000);
	})
/*
	command.add('bb', () => {
		enabled = !enabled
		if (enabled) interval = mod.setInterval(removeBodyBlock, 5000)
		else mod.clearInterval(interval)
